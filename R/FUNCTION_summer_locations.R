...
# 5: add missing dates to te "winter" track
   ...
   winter<-df
   low_threshold_df<-df
   if(any(grepl(".lux", luxfile))|any(grepl("c65_super", luxfile))|any(grepl("c330", luxfile))|any(grepl("c250", luxfile))|any(grepl("c65", luxfile))|any(grepl("f100", luxfile))){light_threshold<-300}else{light_threshold<-50}
   sun_angle_start<- 0
   sun_angle_end<- 0
   ...
   temp<-trn2
   if(is.null(sun_angle_start)){sun_angle_start<-as.numeric(0)}
   if(is.na(sun_angle_start)){temp$sun<-as.numeric(0)}
   if(!is.na(sun_angle_start)){temp$sun<-sun_angle_start}
   if(is.null(sun_angle_end)){sun_angle_end<-sun_angle_start}
   if(!is.na(sun_angle_end)){temp$sun<-seq(sun_angle_start,sun_angle_end,length.out=length(temp$tFirst))}
   ...
   if(is.null(sun_angle_start)){sun_angle_start<-as.numeric(0)}
   if(is.na(sun_angle_start)){posdata$sun<-as.numeric(0)}
   ...
   if(col_lat>0){(summer_days<-c(98:247))}else{summer_days<-c(285:366,1:60)}
   if(species%in%c("Arctic tern","arctic tern","Sterna paradisaea","sterna paradisaea","ARTE")){(summer_days<-c(285:366,1:60,110:247))}
