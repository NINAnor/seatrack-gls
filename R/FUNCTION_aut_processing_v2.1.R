...
if(!exists("save_filter_plots.dir")){save_filter_plots.dir<-NA}
if(!exists("save_sun_plots.dir")){save_sun_plots.dir<-NA}

   show_sun_plot<-TRUE
   if(is.na(save_sun_plots.dir)){show_sun_plot<-FALSE}
   #if(exists("save_sun_plots.dir")){if(!is.na(save_sun_plots.dir)){show_sun_plot<-TRUE}}
   show_filter_plots<-TRUE
   if(is.na(save_filter_plots.dir)){show_filter_plots<-FALSE}
  ...
  if(any(grepl(".lux", luxfile))){lu<-read.table(luxfile,sep="\t",header=F, fill = T, skip = 20)} else {lu<-read.table(luxfile,sep=",",skip=1,header=F, fill = T)}
  if(any(grepl(".lux", luxfile))){ lu$dtime <- datetime_conversion(lu[,1])}else{ lu$dtime <- datetime_conversion(lu[,2])}
  if(any(grepl(".lux", luxfile))){lu$lux <- lu[,2]}else{lu$lux <- lu[,4]}
  ...
  if(any(lu$V1%in% c("ok","OK","Ok"))){ lu<-lu[lu$V1%in%c("ok","OK","Ok"),]}
  trn2<-NULL
  ...
  if(is.null(trn2) & any(grepl(".lux", luxfile)) & logger_model%in%c("f100","c65")){lu$lux<-log(lu$lux)}
  ...
  for(i in 1:(nrow(trn2)-2)){
    if(sum(trn2$type[i:(i+2)])==3){trn2$type[i+1]<-2}
    if(sum(trn2$type[i:(i+2)])==6){trn2$type[i+1]<-1}}
  ...
  if(show_filter_plots==TRUE & !is.null(save_filter_plots.dir)){dev.off()}
  ...
  temp<-trn2
                         if(is.null(sun_angle_start)){sun_angle_start<-as.numeric(-3.5)}
                         if(is.na(sun_angle_start)){temp$sun<-as.numeric(-3.5)}
                         if(!is.na(sun_angle_start)){temp$sun<-as.numeric(sun_angle_start)}
                         if(is.null(sun_angle_end)){sun_angle_end<-as.numeric(sun_angle_start)}
                         if(!is.na(sun_angle_end)){temp$sun<-as.numeric(seq(sun_angle_start,sun_angle_end,length.out=length(temp$tFirst)))}
  ...
  if(is.null(noonfilter)){daylengthfilter<-TRUE}
  ...
  if(is.null(noonfilter)){noonfilter<-TRUE}
  if(noonfilter==TRUE){
  par( mfrow = c(1, 1) )
   if(show_filter_plots==TRUE & !is.null(save_filter_plots.dir)){
  ...
  if(aut_equinox_periods==TRUE){posdata$eqfilter<-assign_equinox_periods(lats = posdata$lat_smooth2,dates = posdata$tFirst,breedingloc_lat=col_lat,sun=temp$sun)}
  temp<-NULL
  ...
  if(is.null(sun_angle_start)){sun_angle_start<-as.numeric(-3.5)}
  if(is.na(sun_angle_start)){posdata$sun<-as.numeric(-3.5)}
  if(!is.na(sun_angle_start)){posdata$sun<-sun_angle_start}
  if(is.null(sun_angle_end)){sun_angle_end<-sun_angle_start}
  if(!is.na(sun_angle_end)){posdata$sun<-seq(sun_angle_start,sun_angle_end,length.out=length(posdata$tFirst))}
  ...
  if(show_filter_plots==TRUE){
    ...
    #before and after twilight removal
                         ...
                         if(is.null(sun_angle_start)){sun_angle_start<-as.numeric(-3.5)}
                         if(is.na(sun_angle_start)){sun<-as.numeric(-3.5)}
                         if(!is.na(sun_angle_start)){sun<-as.numeric(sun_angle_start)}
                         if(is.null(sun_angle_end)){sun_angle_end<-as.numeric(sun_angle_start)}
                         if(!is.na(sun_angle_end)){sun<-as.numeric(seq(sun_angle_start,sun_angle_end,length.out=length(before_tw_remov$tFirst)))}
    ...
    basis<-cbind(before_tw_remov,latlon)
                         ...
                         if(!is.na(sun_angle_start)){sun<-as.numeric(sun_angle_start)}
                         if(is.null(sun_angle_end)){sun_angle_end<-as.numeric(sun_angle_start)}
                         if(!is.na(sun_angle_end)){sun<-as.numeric(seq(sun_angle_start,sun_angle_end,length.out=length(after_tw_remov$tFirst)))}
    ...
    if(!is.na(sun_angle_start)){sun<-as.numeric(sun_angle_start)}
    if(is.null(sun_angle_end)){sun_angle_end<-as.numeric(sun_angle_start)}
    if(!is.na(sun_angle_end)){sun<-as.numeric(seq(sun_angle_start,sun_angle_end,length.out=length(before_changes$tFirst)))}
    ...
    if(!is.na(sun_angle_start)){sun<-as.numeric(sun_angle_start)}
    if(is.null(sun_angle_end)){sun_angle_end<-as.numeric(sun_angle_start)}
    if(!is.na(sun_angle_end)){sun<-as.numeric(seq(sun_angle_start,sun_angle_end,length.out=length(after_changes$tFirst)))}
  ...
  if(show_sun_plot%in%TRUE){
    ...
    if(show_sun_plot==TRUE & exists("save_sun_plots.dir")){
      tiff(filename=paste(save_sun_plots.dir,logger_yeartracked,"lat vs time",".tiff",sep=""), height = 20, width = 18, units = 'cm',
    ...
    for (i in 1:(length(sun.angles))){
      ...
      if(show_sun_plot==TRUE & exists("save_sun_plots.dir")){dev.off()}


      if(show_sun_plot==TRUE & exists("save_sun_plots.dir") == FALSE){dev.new()}
      if(show_sun_plot==TRUE & exists("save_sun_plots.dir")){
        tiff(filename=paste(save_sun_plots.dir,logger_yeartracked,"sun maps ","aut_",".tiff",sep=""), height = 30, width = 25.5, units = 'cm',
      ...
      legend("top", title="Time of the year",
    ...
    add_summer_locations<-summer_locations(df=posdata,
    ...
    add_winter_locations<-winter_locations(df=posdata,
					   luxfile  = luxfile, #specify directory + filename with light data
                                           speed   = speed,               #flight speed (km/h) #previously tried half speed (speed/2)
                                           boundary.box  = boundary.box,   #distribution limits (c(lonW,lonE,latS,latN)) PS! lons as 0-360 deg if crossing the 180 meridian (otherwise -180 to 180)
                                           loess_filter_k              = loess_filter_k,                 #n interquartile ranges
    ...
    #Re-order columns
    col_order <- c("date_time", "logger_id","logger_model","year_tracked",
