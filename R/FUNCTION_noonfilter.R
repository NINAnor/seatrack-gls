...
for (t in 1:2) {
  cor <- rep(NA, 24)
  for (i in 0:23) {cor[i + 1] <- max(abs((c(mid[test$type == t][1], mid[test$type == t]) + i)%%24 - (c(mid[test$type == t], mid[test$type == t][length(mid)]) + i)%%24), na.rm = T)}
...
if(show_plot==TRUE){
  ...
  legend("top",legend = c("ok","del. twl","noon","pred. noon","midnight","pred.midnight"),horiz = TRUE,
