daylengthfilter <- function(df,show_plot){
			     ...
			     #long
                             if(TRUE %in% (c(5,6,7,8,9) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(4,5,6,7,8,9)])>15){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(5,6,7,8,9)]<-23.5}}
                             if(TRUE %in% (c(5,6,7,8) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(5,6,7,8)])<=7){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(4,5,6,7,8)]<-0.5}}
                             if(TRUE %in% (c(10,11,12,1,2) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(10,11,12,1,2)])>15){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(10,11,12,1,2)]<-23.5}}
                             if(TRUE %in% (c(11,12,1,2) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){    
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(11,12,1,2)])<=7){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(11,12,1,2)]<-0.5}}
			     ...
			     #long	
                             if(TRUE %in% (c(5,6,7,8) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(5,6,7,8)])>17){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(5,6,7,8)]<-23.5}}
                             if(TRUE %in% (c(4,5,6,7,8) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(4,5,6,7,8,9)])<=15){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(4,5,6,7,8)]<-0.5}}
                             if(TRUE %in% (c(11,12,1) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(11,12,1,2)])>17){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(11,12,1)]<-23.5}}
                             if(TRUE %in% (c(10,11,12,1,2) %in% unique(month(full_test$date[!is.na(full_test$tFirst)])))){    
                               if(median(full_test$daylength[!is.na(full_test$daylength) & month(full_test$date)%in%c(10,11,12,1,2)])<=15){full_test$daylength[is.na(full_test$daylength) & month(full_test$date)%in%c(10,11,12,1,2)]<-0.5}}
