...
for(i in 1:length(ok$time[ok$type==2])){
  ...
  ###########################
                          ...
                          if(species%in%c("Common eider","common eider","Somateria mollissima","somateria mollissima", 
					  ...
					  "Snow petrel","snow petrel","Pagodroma nivea","pagodroma nivea")){
                          ...
                          for(i in 1:length(unique(rise$date))){rise$remove[rise$date%in%unique(rise$date)[i] & rise$time_mins%in%min(rise$time_mins[rise$date%in%unique(rise$date)[i]])]<-FALSE}
                          rise$date<-NULL
                          ...
                          for(i in 1:length(unique(set$date))){set$remove[set$date%in%unique(set$date)[i] & set$time_mins%in%max(set$time_mins[set$date%in%unique(set$date)[i]])]<-FALSE}
                          set$date<-NULL   
  ...
  #fill predictions for dates with no light with twilights that makes up a daylength of 0 hours:
    ...
    if(breedingloc_lat<0){summer_months<-c(10,11,12,1,2,3)}
...
#  }
  #define "winter" months.
    ...
    if(breedingloc_lat<0){winter_months<-c(4,5,6,7,8,9)}
  ...
  if(is.na(sun_angle_start)){sun_angle_start<-as.numeric(-3.5)}
  if(is.null(sun_angle_end)){sun_angle_end<-sun_angle_start}
  if(is.na(sun_angle_end)){sun_angle_end<-sun_angle_start}
...
#14april2023: er det noe krøll her? Er logikken uansett feil?  
  ...
  if(show_plot==TRUE){
    ...
    legend("top",legend = c("ok","del. twl","retained twl","pred. twl","pred.twl (nest)"),horiz = TRUE,
